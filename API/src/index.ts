...
async function validateGame(gameName: string): Promise<[boolean, string, any, any]> {
  const payload = {"query": gameName,"attributesToHighlight":[],"attributesToSnippet":[],"facets":["tags"],"facetFilters":[["appType:Game"]],"hitsPerPage":100,"attributesToRetrieve":["lastUpdated","name","objectID","followers","oslist","releaseYear","tags","technologies","userScore"],"filters":"(objectID:567380)"}
  ...
  if (response.ok) {
    ...
    if (response['hits'].length > 0) {
      return [true, 'Game Found', null, null];
    }
    else
    {
...
app.get('/fetch', async(c) => {
  ...
  if (gameName != undefined) {
    ...
    if (validateResponse[0]) {
      return c.text(gameName + " was found in the games list")
    }
    else
    {
